...
class SettingsSidebarHandlers:
    ...
    def _apply_output_container_settings(self, ffmpeg):
        ffmpeg.output_container = GeneralSettings.CONTAINERS_UI[self.container_combobox.get_active()]

    def _apply_frame_rate_settings(self, general_settings):
        if self.custom_frame_rate_radiobutton.get_active():
            general_settings.frame_rate = GeneralSettings.FRAME_RATES[self.fps_combobox.get_active()]
    ...
    def _setup_output_container(self, output_container, is_output_container_set):
        if is_output_container_set:
            self.container_combobox.set_active(GeneralSettings.CONTAINERS_UI.index(output_container))
        else:
    ...
    def _setup_audio_codec(self, audio_settings):
        if not self.inputs_page_handlers.is_apply_all_selected():
            ffmpeg = self.inputs_page_handlers.get_selected_row_ffmpeg()
    ...
    def _rebuild_mp4_video_codec_combobox(self):
        if NvidiaHelper.is_nvenc_supported():
            self._rebuild_video_codec_combobox(GeneralSettings.VIDEO_CODECS_MP4_NVENC_UI)
        else:
            self._rebuild_video_codec_combobox(GeneralSettings.VIDEO_CODECS_MP4_UI)
    ...
    def _rebuild_mp4_audio_codec_combobox(self):
        self._rebuild_audio_codec_combobox(GeneralSettings.AUDIO_CODECS_MP4_UI)
    ...
    def _setup_mp4_video_codec_widgets(self, video_codec_text):
        if NvidiaHelper.is_nvenc_supported():
            if video_codec_text in GeneralSettings.VIDEO_CODECS_MP4_NVENC_UI:
                self.video_codec_combobox.set_active(
                    GeneralSettings.VIDEO_CODECS_MP4_NVENC_UI.index(video_codec_text))

                return
        else:
            if video_codec_text in GeneralSettings.VIDEO_CODECS_MP4_UI:
                self.video_codec_combobox.set_active(
                    GeneralSettings.VIDEO_CODECS_MP4_UI.index(video_codec_text))
    ...
    def _setup_mp4_audio_codec_widgets(self, audio_codec_text):
        if audio_codec_text in GeneralSettings.AUDIO_CODECS_MP4_UI:
            self.audio_codec_combobox.set_active(
                GeneralSettings.AUDIO_CODECS_MP4_UI.index(audio_codec_text))
        else:
            self.is_widgets_setting_up = False
    ...
    def _rebuild_mkv_video_codec_combobox(self):
        if NvidiaHelper.is_nvenc_supported():
            self._rebuild_video_codec_combobox(GeneralSettings.VIDEO_CODECS_MKV_NVENC_UI)
        else:
            self._rebuild_video_codec_combobox(GeneralSettings.VIDEO_CODECS_MKV_UI)

    def _rebuild_mkv_audio_codec_combobox(self):
        self._rebuild_audio_codec_combobox(GeneralSettings.AUDIO_CODECS_MKV_UI)

    def _setup_mkv_video_codec_widgets(self, video_codec_text):
        if NvidiaHelper.is_nvenc_supported():
            if video_codec_text in GeneralSettings.VIDEO_CODECS_MKV_NVENC_UI:
                self.video_codec_combobox.set_active(
                    GeneralSettings.VIDEO_CODECS_MKV_NVENC_UI.index(video_codec_text))

                return
        else:
            if video_codec_text in GeneralSettings.VIDEO_CODECS_MKV_UI:
                self.video_codec_combobox.set_active(
                    GeneralSettings.VIDEO_CODECS_MKV_UI.index(video_codec_text))
    ...
    def _setup_mkv_audio_codec_widgets(self, audio_codec_text):
        if audio_codec_text in GeneralSettings.AUDIO_CODECS_MKV_UI:
            self.audio_codec_combobox.set_active(
                GeneralSettings.AUDIO_CODECS_MKV_UI.index(audio_codec_text))
        else:
            self.is_widgets_setting_up = False
            self._reset_and_signal_audio_codec_combobox()
            self.is_widgets_setting_up = True
    ...
    def _rebuild_ts_video_codec_combobox(self):
        if NvidiaHelper.is_nvenc_supported():
            self._rebuild_video_codec_combobox(GeneralSettings.VIDEO_CODECS_TS_NVENC_UI)
        else:
            self._rebuild_video_codec_combobox(GeneralSettings.VIDEO_CODECS_TS_UI)

    def _rebuild_ts_audio_codec_combobox(self):
        self._rebuild_audio_codec_combobox(GeneralSettings.AUDIO_CODECS_TS_UI)

    def _setup_ts_video_codec_widgets(self, video_codec_text):
        if NvidiaHelper.is_nvenc_supported():
            if video_codec_text in GeneralSettings.VIDEO_CODECS_TS_NVENC_UI:
                self.video_codec_combobox.set_active(
                    GeneralSettings.VIDEO_CODECS_TS_NVENC_UI.index(video_codec_text))

                return
        else:

            if video_codec_text in GeneralSettings.VIDEO_CODECS_TS_UI:
                self.video_codec_combobox.set_active(
                    GeneralSettings.VIDEO_CODECS_TS_UI.index(video_codec_text))
    ...
    def _setup_ts_audio_codec_widgets(self, audio_codec_text):
        if audio_codec_text in GeneralSettings.AUDIO_CODECS_TS_UI:
            self.audio_codec_combobox.set_active(
                GeneralSettings.AUDIO_CODECS_TS_UI.index(audio_codec_text))
        else:
            self.is_widgets_setting_up = False
            self._reset_and_signal_audio_codec_combobox()
            self.is_widgets_setting_up = True
    ...
    def _rebuild_webm_video_codec_combobox(self):
        self._rebuild_video_codec_combobox(GeneralSettings.VIDEO_CODECS_WEBM_UI)

    def _rebuild_webm_audio_codec_combobox(self):
        self._rebuild_audio_codec_combobox(GeneralSettings.AUDIO_CODECS_WEBM_UI)

    def _setup_webm_video_codec_widgets(self, video_codec_text):
        if video_codec_text in GeneralSettings.VIDEO_CODECS_WEBM_UI:
            self.video_codec_combobox.set_active(
                GeneralSettings.VIDEO_CODECS_WEBM_UI.index(video_codec_text))
        else:
            self.is_widgets_setting_up = False
            self._reset_and_signal_video_codec_combobox()
            self.is_widgets_setting_up = True
    ...
    def _setup_webm_audio_codec_widgets(self, audio_codec_text):
        if audio_codec_text in GeneralSettings.AUDIO_CODECS_WEBM_UI:
            self.audio_codec_combobox.set_active(
                GeneralSettings.AUDIO_CODECS_WEBM_UI.index(audio_codec_text))
        else:
            self.is_widgets_setting_up = False
            self._reset_and_signal_audio_codec_combobox()
            self.is_widgets_setting_up = True
