...
class NvencMultiPassSignal:
    ...
    def on_nvenc_multi_pass_combobox_changed(self, nvenc_multi_pass_combobox):
        """
        Applies the Multi-Pass option and updates the preview page.

        :param nvenc_multi_pass_combobox: Combobox that emitted the signal.
        """
        if self.nvenc_handlers.is_widgets_setting_up:
            return

        multi_pass_index = nvenc_multi_pass_combobox.get_active()
        codec_settings = None
