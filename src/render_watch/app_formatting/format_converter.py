...
def _generate_hours_timecode_portion(hours):
    if hours == 0:
...
def _generate_minutes_timecode_portion(minutes):
    if minutes == 0:
...
def _generate_seconds_timecode_portion(seconds):
    if seconds == 0:
...
def _get_gigabytes_from_bytes(bytes_value):
    gigabytes = round(bytes_value / GIGABYTE_IN_BYTES, 1)
    return str(gigabytes) + 'GB'


def _get_megabytes_from_bytes(bytes_value):
