...
class OutputChooserSignal:
    ...
    def on_output_chooserbutton_file_set(self, file_chooser_button):
        """Applies the new output path to all inputs.
        If the file path isn't accessible, then the user is notified and the old path is used instead.

        :param file_chooser_button:
            File chooser button that emitted the signal.
        """
        ...
        if not directory_helper.is_directory_accessible(folder_path):
            ...
            return
        self._apply_new_output_directory(folder_path)

    def _apply_new_output_directory(self, folder_path):
        # Applies the new directory to the application preferences and to all inputs on the inputs page.
