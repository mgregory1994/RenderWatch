...
class X265PModeSignal:
    """
    Handles the signal emitted when the x265 PMode option is changed.
    """
    ...
    def on_x265_pmode_checkbutton_toggled(self, x265_pmode_checkbutton):
        ...
        :param x265_pmode_checkbutton: Checkbutton that emitted the signal.
        """
        if self.x265_handlers.is_widgets_setting_up:
            return

        is_pmode_enabled = x265_pmode_checkbutton.get_active()

        for row in self.inputs_page_handlers.get_selected_rows():
            ffmpeg = row.ffmpeg
            ffmpeg.video_settings.pmode = is_pmode_enabled
