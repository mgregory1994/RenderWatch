...
class NvencBRefModeSignal:
    """
    Handles the signal emitted when the NVENC B-Ref Mode option is changed.
    """
    ...
    def on_nvenc_b_ref_mode_combobox_changed(self, nvenc_b_ref_mode_combobox):
        """
        Applies the B-Ref Mode option and updates the preview page.

        :param nvenc_b_ref_mode_combobox: Combobox that emitted the signal.
        """
        if self.nvenc_handlers.is_widgets_setting_up:
            return

        bref_mode_index = nvenc_b_ref_mode_combobox.get_active()
        codec_settings = None
