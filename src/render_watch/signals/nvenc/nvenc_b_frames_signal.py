...
class NvencBFramesSignal:
    """
    Handles the signal emitted when the NVENC B-Frames option is changed.
    """
    ...
    def on_nvenc_b_frames_spinbutton_value_changed(self, nvenc_b_frames_spinbutton):
        """
        Applies the B-Frames option and updates the preview page.

        :param nvenc_b_frames_spinbutton: Spinbutton that emitted the signal.
        """
        if self.nvenc_handlers.is_widgets_setting_up:
            return

        b_frames_value = nvenc_b_frames_spinbutton.get_value_as_int()
        codec_settings = None

        for row in self.inputs_page_handlers.get_selected_rows():
            ffmpeg = row.ffmpeg
            ffmpeg.video_settings.b_frames = b_frames_value
