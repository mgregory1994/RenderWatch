...
class PerCodecX265Row(Gtk.ListBoxRow):
    """
    Creates a Gtk.ListboxRow for the per codec x265 option in the Application Preferences Dialog.
    """
    ...
    def _setup_widgets(self, application_preferences):
        ...
        self.per_codec_label.set_text('x265 Tasks:')
        self.per_codec_subtext_label = gtk_builder.get_object('per_codec_subtext_label')
        self.per_codec_subtext_label.set_text('Number of tasks to run for the x265 codec')
        ...
        self.per_codec_combobox.set_active(ApplicationPreferences.PER_CODEC_TASKS_VALUES.index(
            str(application_preferences.per_codec_parallel_tasks['x265'])))

        self.add(self.per_codec_listbox_row_box)

        self.per_codec_combobox.connect('changed',
                                        self.per_codec_parallel_tasks_signal.on_per_codec_x265_combobox_changed)
